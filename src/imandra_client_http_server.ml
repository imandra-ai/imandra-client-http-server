...
let logic_eval_result ~(src_syntax: Api.src_syntax) src =
  let open Api in
  let syntax = match src_syntax with
    | Reason -> Imandra_syntax.Syntax.Reason
    | Iml -> Imandra_syntax.Syntax.Iml
  in
  (* Note, if the string contains #program;; this can still run program mode stuff *)
      Imandra_util.Pconfig.(with_mode_assigned ~to_:(State.Logic) (fun () ->
...
let print_instance (printer_details: Api.Request.printer_details) =
  ...
  Imandra_util.Pconfig.(with_mode_assigned ~to_:(State.Program) (fun () ->
      Imandra_syntax.Syntax.with_local Imandra_syntax.Syntax.Iml (fun () ->
          LI.eval_string_returning_string (Printf.sprintf "%s CX.%s" name cx_var_name))))

let spec = Spec.spec

let transform_verify_result ~(printer_details : Api.Request.printer_details option) r =
  let open L.Verify in
  let mapped = match r with
  | (V_proved _) ->
  ...
  | (V_proved_upto {upto;_}) ->
     Api.Response.V_proved_upto
       (match upto with
        | L.Event.Upto_steps s -> Api.Response.Upto_steps s
        | L.Event.Upto_bound b -> Api.Response.Upto_bound b)
  ...
  | (V_unknown {reason;_}) ->
    Api.Response.V_unknown
      Api.Response.(
        { unknown_reason = reason })
  in
  mapped |> E.Response.verify_result

let transform_instance_result ~(printer_details : Api.Request.printer_details option) r =
  ...
  let mapped = match r with
    ...
    | (I_unknown {reason;_}) ->
       Api.Response.I_unknown
         Api.Response.({ unknown_reason = reason})
  ...
  let parsed =
    (Decoders_yojson.Basic.Decode.decode_string decoder json_str
     |> CCResult.map_err (fun e ->
         (bad_request (CCFormat.asprintf "%a" Decoders_yojson.Basic.Decode.pp_error e))
       )
    )
  in
  match parsed |> CCResult.map f with
  | Ok r -> r
  | Error r -> r
...
let map_hint (h : Api.Request.Hints.t) =
  ...
  Imandra_surface.Hints.(make (match h.method_ with
...
let handle method_ path body =
  ...
  | (`POST, "/verify/by-src") ->
    with_decoded_json D.Request.verify_req_src body (fun req_src ->
        ...
        let to_eval = (Printf.sprintf "let %s = %s" s src)
        in
        match logic_eval_result ~src_syntax:req_src.syntax to_eval () with
        | Error e ->
          error_response e
        ...
        | Ok _ ->
          try
            let hints =
              req_src.hints
              |> CCOpt.map (fun h -> map_hint h)
            in
            L.Verify.top ~reflect:true ~quiet:true ?hints s
            |> transform_verify_result ~printer_details:req_src.instance_printer
            |> ok_response
  ...
  | (`POST, "/verify/by-name") ->
    with_decoded_json D.Request.verify_req_name body (fun req_name ->
        try
          ...
          |> transform_verify_result ~printer_details:req_name.instance_printer
          |> ok_response
        with e -> error_response e
      )
    ...
    with_decoded_json D.Request.instance_req_src body (fun req_src ->
        ...
        | Ok _ ->
          try
            ...
            |> transform_instance_result ~printer_details:req_src.instance_printer
            |> ok_response
          with e -> error_response e
      )
    ...
    with_decoded_json D.Request.instance_req_name body (fun req_name ->
        try
          ...
          let json =
            ...
            |> transform_instance_result ~printer_details:req_name.instance_printer
          in
          ok_response json
        with e -> error_response e
      )
...
let speclist =
  Arg.align
    [ ("-server", Arg.Set_string server_name, " Binary to use as imandra server, defaults to imandra_server")
    ; ("-reason", Arg.Set reason, " enable reason syntax")
    ; ("-require", Arg.String (fun s->req:=s::!req), " Require given library")
    ; ("-timeout", Arg.Int Imandra_util.Pconfig.State.Set.timeout, " Timeout (in ms) for the solver")
    ; ("-port", Arg.Set_int port, (Printf.sprintf " port to listen on (default %d)" default_port))
    ; ("-dir", Arg.String (fun s->dirs := s:: !dirs), " add directory to load path")
    ...
    body |> Cohttp_lwt.Body.to_string >>= (fun body_str ->
        ...
        |> Lwt.map (fun ((res, body) : (Cohttp.Response.t * _)) ->
            (Cohttp.Response.make
              ...
              ~headers: (Cohttp.Header.add res.Response.headers "Access-Control-Allow-Origin" "*")
              ()
            , body
            )
          )
...
let () =
  Arg.parse speclist print_endline "Query Imandra via HTTP.\n\ncurl localhost:<port>/spec for an open api 3.0 spec of available endpoints and their parameters.\n";
  if !lock >= 0 then LI.set_lockdown !lock;
  List.iter LI.require_lib_at_init (List.rev !req);
  let syntax = if !reason then Imandra_syntax.Syntax.Reason else Imandra_syntax.Syntax.Iml in
